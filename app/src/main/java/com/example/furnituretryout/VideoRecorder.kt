...
class VideoRecorder(
    ...
    private fun buildFile(): File? {
        ...
        val path =
            Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_MOVIES)?.absolutePath + "/TryOutFurniture/${date}_video.mp4"
        val file = File(path)
    ...
    private fun buildFileApi29(): FileDescriptor? {
        ...
        return activity.contentResolver.openAssetFileDescriptor(
            uri ?: return null,
            "w"
        )?.fileDescriptor
    }
